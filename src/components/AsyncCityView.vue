      ...
      <div class="mx-8 text-white">
        ...
        <div class="flex gap-5 overflow-x-scroll">
          <div
            ...
            v-for="hourData in weatherForecastResults.forecast.forecastday[0].hour"
          ...
          >
            ...
            <p class="text-xs">{{ hourData.chance_of_rain }}%</p>
          </div>
        </div>
      </div>
    </div>
...
const getWeatherForecast = async () => {
  try {
    const weatherForecast = await axios.get(
      `http://api.weatherapi.com/v1/forecast.json?key=${import.meta.env.VITE_WEATHERAPI_KEY}&q=${route.params.city}&qdays=3`
    )
